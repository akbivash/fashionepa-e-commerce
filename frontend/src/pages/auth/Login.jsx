...
const Login = () => {
  ...
  const { currentUser, loading, error, errorMsg } = useSelector(
    (state) => state.user
  );
  ...
  return (
    <>
      <div className=" flex justify-center min-h-screen   p-3 ">
        {currentUser && !error && (
          <span className="py-7">You are logged in</span>
        )}
        {!currentUser && (
          <form
            action=""
          ...
          >
            ...
            />
            <button
              className="text-yellow-dark text-left"
        ...
        {isModal && (
          <Modal>
            ...
            <div className="flex gap-4 items-center">
              <Link to="/register" className="bg-green-dark p-2 ">
                Ok
              </Link>
              <span
                className="text-yellow-light text-2xl cursor-pointer"
